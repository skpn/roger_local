...
sudo iptables -A INPUT -j LOG --log-level 4 --log-prefix=iptables_input_reject:
sudo iptables -A INPUT -j REJECT
sudo iptables -A OUTPUT -j LOG --log-level 4 --log-prefix=iptables_output_reject:
sudo iptables -A OUTPUT -j REJECT
sudo iptables -A FORWARD -j LOG --log-level 4 --log-prefix=iptables_forward_reject:
...
### accept 5 icmp protocol outbound/inbound ping packets per second up to 1000
### packets
...
sudo iptables -I INPUT -p tcp -m multiport --dports 50000,80,443,25 -m conntrack ! --ctstate INVALID --ctstate NEW -m limit --limit 1/s --limit-burst 300 -j ACCEPT
sudo iptables -I INPUT -p tcp -m multiport --dports 50000,80,443,25 -m conntrack ! --ctstate INVALID --ctstate NEW -m limit --limit 1/s --limit-burst 300 -j LOG --log-level 4 --log-prefix=iptables_new:

### accept loopback packets
ip_addr=$(ip addr show enp0s3 | awk '{ if ($1 == "inet") print $2}')
sudo iptables -I INPUT -i lo -s 127.0.0.1 -j ACCEPT
...
sudo iptables -I INPUT -p tcp -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
sudo iptables -I INPUT -p tcp -m conntrack --ctstate ESTABLISHED,RELATED -j LOG --log-level 4 --log-prefix=iptables_er_accept:
